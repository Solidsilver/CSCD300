...
public class Tester {
	...
	public int countSpace(String str) {	
		if (!str.contains(" ")) {
			...
			return countSpace(str.substring(str.indexOf(" ")+1)) + 1;
		}
	}
	...
	public int div(int m, int n) throws IllegalArgumentException {
		if (n==0) {
			throw new IllegalArgumentException("THE WORLD WILL END!!!");
		}
		if (n > m) {
			return 0;
		}
	...
	private boolean isSum24(int arr[], int targetSum) {
		...
		if (arr.length == 1) {
			if (arr[0] == targetSum) return true;
			return false;
		}
		int[] b = Arrays.copyOfRange(arr, 1, arr.length);
		return isSum24(b, targetSum - arr[0]);
	}
